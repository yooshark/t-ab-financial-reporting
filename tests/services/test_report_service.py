...
async def test_get_report_total_calculation_positive(report_service, mock_uow):
    params = ReportQueryParams()
    mock_metrics = {"amount_total": Decimal("1500.50")}
    mock_uow.transaction_repo.get_base_metrics.return_value = mock_metrics
...
async def test_get_report_average_calculation_positive(report_service, mock_uow):
...
async def test_get_report_min_max_calculation_positive(report_service, mock_uow):
...
async def test_get_report_daily_aggregations_positive(report_service, mock_uow):
    params = ReportQueryParams(include_daily_shift=True, include_avg=True)
    base_metrics = {"change_daily_shift": Decimal("1000.00")}
    daily_metrics = [
        {
            "date": date(2024, 1, 1),
            "change_daily_shift": Decimal("100.00"),
        },
        {
            "date": date(2024, 1, 2),
            "change_daily_shift": Decimal("200.00"),
        },
    ]

    mock_uow.transaction_repo.get_base_metrics.return_value = base_metrics
    mock_uow.transaction_repo.get_daily_metrics.return_value = daily_metrics

    result = await report_service.get_report(params)

    assert len(result["daily"]) == 2
    assert result["daily"][0]["date"] == date(2024, 1, 1)
    assert result["daily"][0]["change_daily_shift"] == Decimal("100.00")


async def test_get_report_by_country_positive(report_service, mock_uow):
    mock_df = pd.DataFrame({"id": [1, 2, 3], "country": ["USA", "USA", "UK"]})

    with patch("app.services.report_service.read_csv", return_value=mock_df):
        mock_uow.user_repo.get_user_transactions_by_ids.return_value = [
...
async def test_get_report_empty_repo_result(report_service, mock_uow):
    params = ReportQueryParams()
    mock_uow.transaction_repo.get_base_metrics.return_value = {
...
async def test_get_report_by_countries_empty_csv(report_service, mock_uow):
...
async def test_get_report_by_countries_no_users_found(report_service, mock_uow):
